...
function renderMap(stations) {
  ...
  const margin = 0.035;
  const bbox = [
    (lng - margin).toFixed(5),
    (lat - margin).toFixed(5),
    (lng + margin).toFixed(5),
    (lat + margin).toFixed(5)
  ].join("%2C");
...
function renderDetail(stations) {
  ...
  stationDetail.innerHTML = `
    ...
    <div class="detail-actions">
      ...
      <a class="secondary-link" target="_blank" rel="noreferrer" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(station.address)}">Traçar rota</a>
    </div>
  `;
